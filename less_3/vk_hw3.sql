...
CREATE TABLE IF NOT EXISTS `vk`.`likes` (
  ...
  `from_users_id` INT UNSIGNED NOT NULL,
  `liked_users_id` INT UNSIGNED NULL DEFAULT NULL,
  `liked_media_id` INT UNSIGNED NULL DEFAULT NULL,
  PRIMARY KEY (`like_id`),
  INDEX `fk_likes_users_from_user_id` (`from_users_id` ASC) VISIBLE,
  INDEX `fk_likes_users_liked_user_id` (`liked_users_id` ASC) VISIBLE,
  INDEX `fk_likes_media1_liked_media_id` (`liked_media_id` ASC) VISIBLE,
  CONSTRAINT `fk_likes_users_from_user_id`
    FOREIGN KEY (`from_users_id`)
    REFERENCES `vk`.`users` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_likes_users_liked_user_id`
    FOREIGN KEY (`liked_users_id`)
    REFERENCES `vk`.`users` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
